...
const validateUser = async (req, res) => {
    ...
    try {
        ...
        if (user.role === 'floor leader') {
            ...
            return res.render('welcome', {user, floor_leader, userRole: req.session.userRole, userId: req.session.userId, 
            ...
            return res.render('welcome', {user, building_leader, userRole: req.session.userRole, userId: req.session.userId, 
                floorNum: req.session.floorNum, buildingNum: req.session.bldgNum});

        } else if (user.role === 'admin') {
            const admin = await Admin.findOne({user_id: user._id}).lean();
            return res.render('welcome', {user, admin, userRole: req.session.userRole, userId: req.session.userId});

        } else if (user.role === 'tenant') {
            return res.render('welcome', {user, userRole: req.session.userRole, userId: req.session.userId});

        } else if (user.role === 'finance') {
            const finance = await Finance.findOne({user_id: user._id}).lean();
            return res.render('welcome', {user, finance, userRole: req.session.userRole, userId: req.session.userId});
